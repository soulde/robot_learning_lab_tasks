"""MagicLab MagicDog velocity environment configurations."""
...
from robot_learning_lab_zoo.assets.mjlab.magiclab_magicdog import (
    MAGICDOG_ACTION_SCALE, MAGICDOG_CFG, MAGICDOG_FOOT_BODY_NAMES,
    MAGICDOG_FOOT_GEOM_NAMES, MAGICDOG_FOOT_SITE_NAMES, MAGICDOG_JOINT_NAMES,
)

from robot_learning_lab_tasks.tasks.mjlab.velocity import rewards

BASE_BODY = "base"
FOOT_BODIES = MAGICDOG_FOOT_BODY_NAMES
FOOT_SITES = MAGICDOG_FOOT_SITE_NAMES
FOOT_GEOMS = MAGICDOG_FOOT_GEOM_NAMES


def magicdog_actions_cfg():
    return {"joint_pos": JointPositionActionCfg(entity_name="robot", actuator_names=MAGICDOG_JOINT_NAMES,
        scale=MAGICDOG_ACTION_SCALE,
        clip={".*": (-100.0, 100.0)}, use_default_offset=True, preserve_order=True)}


def magicdog_rewards_cfg():
    return {
        ...
        "stand_still": RewardTermCfg(func=rewards.stand_still, weight=-2.0,
            params={"command_name": "twist", "asset_cfg": SceneEntityCfg("robot", joint_names=MAGICDOG_JOINT_NAMES)}),
        ...
        "joint_mirror": RewardTermCfg(func=rewards.joint_mirror, weight=-0.05,
            params={"asset_cfg": SceneEntityCfg("robot", joint_names=MAGICDOG_JOINT_NAMES),
...
def magicdog_rough_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
    ...
    cfg.rewards = magicdog_rewards_cfg()
    ...
    cfg.actions = magicdog_actions_cfg()
...
def magicdog_flat_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
    cfg = magicdog_rough_env_cfg(play=play); assert cfg.scene.terrain is not None
