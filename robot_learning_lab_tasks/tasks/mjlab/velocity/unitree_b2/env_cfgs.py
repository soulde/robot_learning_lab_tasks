...
from robot_learning_lab_zoo.assets.mjlab.unitree_b2 import (
    UNITREE_B2_ACTION_SCALE, UNITREE_B2_CFG, UNITREE_B2_FOOT_BODY_NAMES,
    UNITREE_B2_FOOT_GEOM_NAMES, UNITREE_B2_FOOT_SITE_NAMES, UNITREE_B2_JOINT_NAMES,
)

from robot_learning_lab_tasks.tasks.mjlab.velocity import rewards

BASE_BODY = "base_link"
FOOT_BODIES = UNITREE_B2_FOOT_BODY_NAMES
FOOT_SITES = UNITREE_B2_FOOT_SITE_NAMES
FOOT_GEOMS = UNITREE_B2_FOOT_GEOM_NAMES


def unitree_b2_actions_cfg():
    return {"joint_pos": JointPositionActionCfg(entity_name="robot", actuator_names=UNITREE_B2_JOINT_NAMES,
        scale=UNITREE_B2_ACTION_SCALE,
        clip={".*": (-100.0, 100.0)}, use_default_offset=True, preserve_order=True)}


def unitree_b2_rewards_cfg():
    return {
        ...
        "stand_still": RewardTermCfg(func=rewards.stand_still, weight=-2.0,
            params={"command_name": "twist", "asset_cfg": SceneEntityCfg("robot", joint_names=UNITREE_B2_JOINT_NAMES)}),
        ...
        "joint_mirror": RewardTermCfg(func=rewards.joint_mirror, weight=-0.05,
            params={"asset_cfg": SceneEntityCfg("robot", joint_names=UNITREE_B2_JOINT_NAMES),
        ...
        "feet_height_body": RewardTermCfg(func=rewards.feet_height_body, weight=-5.0,
            params={"command_name": "twist", "asset_cfg": SceneEntityCfg("robot", body_names=FOOT_BODIES),
                    "target_height": -0.4, "tanh_mult": 2.0}),
...
def unitree_b2_rough_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
    cfg = make_velocity_env_cfg()
    cfg.sim.mujoco.ccd_iterations = 500; cfg.sim.contact_sensor_maxmatch = 500
    cfg.sim.nconmax = 500; cfg.sim.njmax = 1000
    cfg.scene.entities = {"robot": deepcopy(UNITREE_B2_CFG)}
    ...
    cfg.rewards = unitree_b2_rewards_cfg()
    ...
    cfg.actions = unitree_b2_actions_cfg()
...
def unitree_b2_flat_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
    cfg = unitree_b2_rough_env_cfg(play=play); assert cfg.scene.terrain is not None
