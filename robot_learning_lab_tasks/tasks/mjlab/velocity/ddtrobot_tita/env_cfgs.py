"""DDTRobot Tita velocity environment configurations."""
...
from robot_learning_lab_zoo.assets.mjlab.ddtrobot import DDTROBOT_TITA_CFG, TITA_WHEEL_BODIES

from robot_learning_lab_tasks.tasks.mjlab.velocity import observations, rewards

LEG_JOINTS = tuple(f"joint_{side}_leg_{index}" for side in ("right", "left") for index in (1, 2, 3))
WHEEL_JOINTS = tuple(f"joint_{side}_leg_4" for side in ("right", "left"))
ALL_JOINTS = LEG_JOINTS + WHEEL_JOINTS
WHEEL_SITES = tuple(f"{name}_site" for name in TITA_WHEEL_BODIES)
WHEEL_GEOMS = tuple(f"{name}_collision" for name in TITA_WHEEL_BODIES)


def ddtrobot_tita_actions_cfg():
    """Build the M20 policy action mapping."""
    return {
        "joint_pos": JointPositionActionCfg(
            entity_name="robot", actuator_names=LEG_JOINTS,
            scale={".*_leg_1": 0.125, "^(?!.*_leg_1).*": 0.25},
...
def ddtrobot_tita_rough_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
    cfg = make_velocity_env_cfg()
    cfg.sim.mujoco.ccd_iterations = 500
    cfg.sim.contact_sensor_maxmatch = 500
    cfg.sim.njmax = 2500
    cfg.sim.nconmax = 1000
    cfg.scene.entities = {"robot": deepcopy(DDTROBOT_TITA_CFG)}
...
def ddtrobot_tita_flat_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
    cfg = ddtrobot_tita_rough_env_cfg(play=play)
