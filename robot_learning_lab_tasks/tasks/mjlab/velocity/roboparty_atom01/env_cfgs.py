"""RoboParty ATOM01 velocity environment configurations."""
...
from robot_learning_lab_zoo.assets.mjlab.roboparty import ROBOPARTY_ATOM01_CFG
...
def roboparty_atom01_actions_cfg():
...
def roboparty_atom01_rewards_cfg():
    return {
        ...
        "air_time": RewardTermCfg(func=mdp.feet_air_time, weight=0.25,
        ...
        "joint_deviation_thigh_l1": RewardTermCfg(func=rewards.joint_deviation_l1, weight=-0.1,
            params={"asset_cfg": SceneEntityCfg("robot", joint_names=(".*thigh_yaw.*", ".*thigh_roll.*"))}),
        "joint_deviation_arms_l1": RewardTermCfg(func=rewards.joint_deviation_l1, weight=-0.1,
...
def roboparty_atom01_rough_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
    cfg = make_velocity_env_cfg()
    cfg.sim.mujoco.ccd_iterations = 500; cfg.sim.contact_sensor_maxmatch = 500
    cfg.sim.nconmax = 500; cfg.sim.njmax = 1000
    cfg.scene.entities = {"robot": deepcopy(ROBOPARTY_ATOM01_CFG)}
    ...
    cfg.actions = roboparty_atom01_actions_cfg(); cfg.rewards = roboparty_atom01_rewards_cfg()
...
def roboparty_atom01_flat_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
    cfg = roboparty_atom01_rough_env_cfg(play=play); assert cfg.scene.terrain is not None
