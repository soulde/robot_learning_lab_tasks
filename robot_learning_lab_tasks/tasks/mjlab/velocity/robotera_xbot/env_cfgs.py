"""RobotEra XBot velocity environment configurations."""
...
from robot_learning_lab_zoo.assets.mjlab.robotera import ROBOTERA_XBOT_CFG
...
def robotera_xbot_rewards_cfg():
    return {
        ...
        "joint_deviation_hip_l1": RewardTermCfg(func=rewards.joint_deviation_l1, weight=-0.01,
            params={"asset_cfg": SceneEntityCfg("robot", joint_names=(".*_leg_yaw_joint", ".*_leg_roll_joint"))}),
        "joint_deviation_arms_l1": RewardTermCfg(func=rewards.joint_deviation_l1, weight=-0.05,
            params={"asset_cfg": SceneEntityCfg("robot", joint_names=(".*_shoulder_.*", ".*_elbow_.*", ".*_arm_.*", ".*_wrist_.*"))}),
...
def robotera_xbot_rough_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
    cfg = make_velocity_env_cfg()
    cfg.sim.mujoco.ccd_iterations = 500; cfg.sim.contact_sensor_maxmatch = 500
    cfg.sim.nconmax = 500; cfg.sim.njmax = 1000
    cfg.scene.entities = {"robot": deepcopy(ROBOTERA_XBOT_CFG)}
    ...
    cfg.actions = robotera_xbot_actions_cfg(); cfg.rewards = robotera_xbot_rewards_cfg()
...
def robotera_xbot_flat_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
    cfg = robotera_xbot_rough_env_cfg(play=play); assert cfg.scene.terrain is not None
