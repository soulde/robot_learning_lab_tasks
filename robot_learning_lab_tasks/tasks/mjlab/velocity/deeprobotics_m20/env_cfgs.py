...
from robot_learning_lab_zoo.assets.mjlab.deeprobotics import DEEPROBOTICS_M20_CFG, M20_WHEEL_BODIES
...
WHEEL_SITES = tuple(f"{name}_site" for name in M20_WHEEL_BODIES)
WHEEL_GEOMS = tuple(f"{name}_collision" for name in M20_WHEEL_BODIES)


def deeprobotics_m20_actions_cfg():
...
def deeprobotics_m20_rewards_cfg():
...
def deeprobotics_m20_rough_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
    cfg = make_velocity_env_cfg()
    cfg.sim.mujoco.ccd_iterations = 500
    cfg.sim.contact_sensor_maxmatch = 500
    cfg.sim.njmax = 2500
    cfg.sim.nconmax = 1000
    cfg.scene.entities = {"robot": deepcopy(DEEPROBOTICS_M20_CFG)}
...
def deeprobotics_m20_flat_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
    cfg = deeprobotics_m20_rough_env_cfg(play=play)
