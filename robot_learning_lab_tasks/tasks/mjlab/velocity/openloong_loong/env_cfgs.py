"""OpenLoong Loong velocity environment configurations."""
...
from robot_learning_lab_zoo.assets.mjlab.openloong import OPENLOONG_CFG
...
def openloong_loong_actions_cfg():
...
def openloong_loong_rewards_cfg():
    return {
        ...
        "joint_deviation_hip_l1": RewardTermCfg(func=rewards.joint_deviation_l1, weight=-0.3,
            params={"asset_cfg": SceneEntityCfg("robot", joint_names=("J_hip_.*_roll", "J_hip_.*_yaw"))}),
        "joint_deviation_ankle_l1": RewardTermCfg(func=rewards.joint_deviation_l1, weight=-0.2,
            params={"asset_cfg": SceneEntityCfg("robot", joint_names=("J_ankle_.*_roll",))}),
        "self_collisions": RewardTermCfg(func=mdp.self_collision_cost, weight=-1.0,
            params={"sensor_name": "self_collision", "force_threshold": 10.0}),
    }


def openloong_loong_rough_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
    cfg = make_velocity_env_cfg()
    cfg.sim.mujoco.ccd_iterations = 500; cfg.sim.contact_sensor_maxmatch = 500
    cfg.sim.nconmax = 500; cfg.sim.njmax = 1000
    cfg.scene.entities = {"robot": deepcopy(OPENLOONG_CFG)}
    ...
    cfg.actions = openloong_loong_actions_cfg(); cfg.rewards = openloong_loong_rewards_cfg()
...
def openloong_loong_flat_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
    cfg = openloong_loong_rough_env_cfg(play=play); assert cfg.scene.terrain is not None
