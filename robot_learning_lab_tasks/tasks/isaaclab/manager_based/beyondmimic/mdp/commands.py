...
class MotionLoader:
    def __init__(self, motion_file: str, body_names: Sequence[str], device: str = "cpu"):
        ...
        if "body_names" in data:
            motion_body_names = list(data["body_names"])
        else:
            motion_body_names = [f"body_{i}" for i in range(self._body_pos_w.shape[1])]

        self._body_indexes = torch.tensor(
            [motion_body_names.index(name) for name in body_names], dtype=torch.long, device=device
        )
